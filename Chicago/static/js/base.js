  ...
  $.ajax({ url: "/obtenernotificaciones/", success: function(json) {
    ...
    for (var i = 0; i < json.length; i++) {
      if (json[i].tipo == true){
        ...
        $("#noti-" + (i + 1).toString()).html(
          "<span class='text-success'>"+
            "<strong>"+
            "<i id='status' style='color:green' class='fa fa-chevron-circle-up'></i> "+ json[i].contenido+
            "<i</>"+
            "</strong>"+
          "</span>"+
          "<div class='dropdown-message small'>Tu vídeo ahora se encuentra visible para todos</div>"+
          ...
          $("#noti-" + (i + 1).toString()).html(
            "<span class='text-danger'>"+
              "<strong>"+
              "<i id='status' style='color:red' class='fa fa-exclamation-triangle'></i> "+ json[i].contenido+
              "<i</>"+
              "</strong>"+
            "</span>"+
            "<div class='dropdown-divider'></div>"
        ...
        $("#noti-" + (i + 1).toString()).html(
          "<span class='text-info'>"+
            "<strong>"+
            "<i id='status' style='color:info' class='fa fa-refresh fa-spin fa-2x fa-fw'></i>"+json[i].contenido+
            "<i</>"+
            "</strong>"+
          "</span>"+
